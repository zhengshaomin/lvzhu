...
import static Min.app.plus.utils.MyUtils.maskPhoneNumber;
...
public class Store_Order_Processed extends Fragment {
    ...
    private View view;
    private ListView order_processed_listview;
    List<Order> mOrderlist = new ArrayList<Order>();
    private String user_id,goods_user,order_id;
    private SmartRefreshLayout order_processed_srlControl;

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if (view==null){
            view=inflater.inflate(R.layout.order_processed, container, false);
        }
        return view;
    }
    @Override
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        order_processed_listview =getActivity().findViewById(R.id.order_processed_listview);
        order_processed_srlControl=getActivity().findViewById(R.id.order_processed_srlControl);

        order_processed_listview.setOnScrollListener(new AbsListView.OnScrollListener() {
            @Override
            ...
            public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
                ...
                if (order_processed_listview != null && order_processed_listview.getChildCount() > 0) {
                    // 检查listView第一个item是否可见
                    boolean firstItemVisible = order_processed_listview.getFirstVisiblePosition() == 0;
                    // 检查第一个item的顶部是否可见
                    boolean topOfFirstItemVisible = order_processed_listview.getChildAt(0).getTop() == 0;
                    // 启用或者禁用SwipeRefreshLayout刷新标识
                    enable = firstItemVisible && topOfFirstItemVisible;
                } else if (order_processed_listview != null && order_processed_listview.getChildCount() == 0) {
                    // 没有数据的时候允许刷新
                    enable = true;
                }
        ...
        //监听下拉和上拉状态
        //下拉刷新
        order_processed_srlControl.setOnRefreshListener(refreshlayout -> {
            order_processed_srlControl.setEnableRefresh(true);//启用刷新
            getstore_processed_order();
            order_processed_srlControl.finishRefresh();//结束刷新
        });
        //上拉加载
    ...
    public void getstore_processed_order(){
        ...
        eq1.addWhereEqualTo("state","待处理");//查询待处理的订单
        ...
        bmobQuery.findObjects(new FindListener<Order>() {
            ...
            public void done(List<Order> object, BmobException e) {
                ...
                if (e == null) {
                    ...
                    order_processed_listview.setAdapter(new Store_Order_Processed.ItemListAdapter());
    ...
    class ItemListAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView, ViewGroup parent) {
            final Store_Order_Processed.ItemListAdapter.ViewHolder viewHolder;
            if (convertView == null) {
                viewHolder = new Store_Order_Processed.ItemListAdapter.ViewHolder();
                convertView = getActivity().getLayoutInflater().inflate(R.layout.item_order_store_processed, null);
                ...
                viewHolder = (Store_Order_Processed.ItemListAdapter.ViewHolder) convertView.getTag();
            }
            ...
            viewHolder.phonenumber.setText(maskPhoneNumber(orders.getConsumer().getMobilePhoneNumber()));//电话号码
            viewHolder.remarks.setText(orders.getRemarks());//用户备注
            viewHolder.address.setText(orders.getAddress());//收货地址
            viewHolder.state.setText(orders.getState());//订单状态
            viewHolder.linearLayout.setOnClickListener(new View.OnClickListener(){
                ...
                public void onClick(View p1)
                {
                    ...
                    intent.putExtra("string_order_state","完成");
